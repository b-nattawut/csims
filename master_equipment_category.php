                ...
                <form id="searchFilterForm" name="searchFilterForm">
                    <div class="row g-2 justify-content-start align-items-end">
                        <div class="col-12 col-md-6 col-lg-3 col-xl-3">
                            <button class="btn <?= $canManage ? 'btn-success' : 'btn-secondary' ?> btn-sm px-4 fw-bold shadow-sm text-nowrap w-100 <?= $canManage ? 'js-add-category-btn' : '' ?>"
                                style="height: 31px;"
                                type="button" <?= $canManage ? 'data-bs-toggle="modal" data-bs-target="#addEditCategoryModal"' : 'disabled' ?>
                        ...
                        <div class="col-12 col-md-6 col-lg-3 col-xl-3">
                            <label for="filter_category_name" class="form-label text-muted small mb-1">ชื่อประเภทเครื่องมือ</label>
                            <input type="text" id="filter_category_name" name="filter_category_name" class="form-control form-control-sm" placeholder="ค้นหาชื่อประเภทเครื่องมือ...">
                        </div>
        ...
        <table class="table table-bordered table-hover table-custom table-striped align-middle mb-0">
            <thead style="font-size: 14px;">
                <tr class="text-nowrap text-center">
                    ...
                    <th style="min-width: 160px;">ชื่อประเภทเครื่องมือ</th>
                    <th style="min-width: 100px;">จำนวนเครื่องมือที่ใช้งาน</th>
...
<!-- Modal สำหรับเพิ่มรายการประเภทเครื่องมือใหม่ / แก้ไขรายการเดิม -->
<div class="modal fade" id="addEditCategoryModal" aria-labelledby="addEditCategoryModalLabel" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white py-3 px-4">
                <h5 class="modal-title fw-bold" id="addEditCategoryModalLabel">
            ...
            <div class="modal-body bg-light p-4">
                <form method="post" id="categoryForm" action="./api/Master_equipment_category/save.php" novalidate>
                    <input type="hidden" id="category_id" name="id">

                    <fieldset class="bg-white p-4 rounded-3 shadow-sm border">
                        <div class="row g-3 justify-content-start">
                            <div class="col-12">
                                <label for="category_name" class="form-label fw-bold">ชื่อประเภทเครื่องมือ <span class="text-danger">*</span></label>
                                <div class="input-group input-group-seamless">
                                    <input type="text" id="category_name" name="category_name" class="form-control"
                                        required autocomplete="off" maxlength="255"
                                        placeholder="เช่น เครื่องตรวจหาวัตถุระเบิด, ชุดตรวจสารเสพติด">
                                    <button type="button"
                                        class="btn btn-hw-open"
                                        data-hw-targets="category_name"
            ...
            <div class="modal-footer d-flex justify-content-end bg-white border-top shadow-sm">
                <button type="submit" class="btn btn-success px-4" id="btn_save_all" form="categoryForm">
...
<div class="modal fade" id="viewCategoryDetailModal" tabindex="-1" aria-labelledby="viewCategoryDetailModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl" style="max-width: 90%;">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header bg-primary text-white py-3">
                <h5 class="modal-title fw-bold" id="viewCategoryDetailModalLabel">
                    <i class="fas fa-search me-2"></i> รายละเอียดประเภทเครื่องมือ
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="row">
                            <div class="col-12 mb-3">
                                <label class="text-muted small d-block">ชื่อประเภทเครื่องมือ</label>
                                <span id="view_cat_name" class="fw-bold">-</span>
                            </div>
                            <div class="col-12">
                                <div class="p-3 bg-primary bg-opacity-10 rounded-3 border border-primary border-opacity-25">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="ms-3">
                                            <label class="text-muted small d-block mb-0">จำนวนเครื่องมือที่ใช้งานประเภทนี้</label>
                                            <span id="view_cat_usage" class="fw-bold text-primary">0</span> <span class="small text-muted">รายการ</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-sm-6 border-end border-white">
                                    <div class="d-flex align-items-center" style="min-height: 45px;"> <i class="fas fa-user-plus fa-lg text-secondary me-3"></i>
                                        <div>
                                            ...
                                            <span id="view_cat_create_by" class="text-dark d-block" style="font-size: 0.85rem;">-</span>
                                            <span id="view_cat_create_date" class="italic" style="font-size: 0.75rem;">-</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-sm-6 ps-4">
                                    <div class="d-flex align-items-center" style="min-height: 45px;">
                                        ...
                                        <div>
                                            ...
                                            <span id="view_cat_update_by" class="text-dark d-block" style="font-size: 0.85rem;">-</span>
                                            <span id="view_cat_update_date" class="italic" style="font-size: 0.75rem;">-</span>
                                        </div>
                                    </div>
                                </div>
...
<script>
    ...
    function searchPage(page) {
        ...
        $.ajax({
            url: './api/Master_equipment_category/searchData.php',
            type: 'POST',
            data: formData,
            dataType: 'json',
    ...
    function renderTable(data, offset) {
        ...
        if (data && data.length > 0) {
            data.forEach(function(row, i) {
                // ถ้ามีเครื่องมือใช้งานอยู่ (usage > 0) จะแสดงสีเน้น และอาจคุมปุ่มลบ
                let usageText = '';
                ...
                html += `
                    <tr class="clickable-row cursor-pointer" data-id="${row.id}" title="ดูรายละเอียด">
                        ...
                        <td class="text-start align-middle ">${row.category_name}</td>
                ...
                <tr>
                    <td colspan="6" class="text-center py-5">
                        <div class="text-muted">
                            ...
                            <p class="mb-0 fw-bold">ไม่พบรายการประเภทเครื่องมือในระบบ</p>
    ...
    $(document).on('click', '.clickable-row', function(e) {
        ...
        $.get('./api/Master_equipment_category/getDataByID.php', {
            id: id
        }, function(res) {
            if (res.status === 'success') {
                const d = res.data;
                $('#view_cat_name').text(d.category_name);
                $('#view_cat_usage').text(d.total_usage || 0);
                $('#view_cat_create_by').text(d.creator_name || '-');
                $('#view_cat_create_date').text(d.created_at_full || '-');
                $('#view_cat_update_by').text(d.updater_name || '-');
                $('#view_cat_update_date').text(d.updated_at_full || '');
    ...
    $(document).on('click', '.js-add-category-btn', function() {
        $('#categoryForm')[0].reset();
        $('#category_id').val('');
        $('#categoryModalTitle').text('เพิ่มรายการ');
        $('#addEditCategoryModalLabel').html('<i class="fas fa-plus fa-lg me-2"></i> เพิ่มรายการ');
    ...
    $(document).on('click', '.js-edit-btn', function() {
        ...
        $.get('./api/Master_equipment_category/getDataByID.php', {
            ...
            if (res.status === 'success') {
                ...
                $('#categoryModalTitle').text('แก้ไขประเภทเครื่องมือ');
                $('#addEditCategoryModalLabel').html('<i class="fas fa-edit fa-lg me-2"></i> แก้ไขประเภทเครื่องมือ');
                $('#addEditCategoryModal').modal('show');
            }
        }, 'json');
    });
    ...
    $('#categoryForm').on('submit', function(e) {
        ...
        $.ajax({
            url: './api/Master_equipment_category/save.php',
            type: 'POST',
    ...
    $(document).on('click', '.js-delete-btn', function() {
        ...
        if (usage > 0) {
            Swal.fire({
                ...
                html: `
                <div class="text-center mb-3">ประเภท <b>"${name}"</b> ยังมีการใช้งานอยู่</div>
                <div class="p-3 bg-light rounded border border-warning shadow-sm">
                    มีเครื่องมือในระบบที่ใช้ประเภทนี้อยู่ <b class="text-danger"> ${usage} รายการ</b><br>
                    <small class="text-muted text-start d-block mt-2">* กรุณาย้ายเครื่องมือไปเป็นประเภทอื่น หรือลบเครื่องมือนั้นออกก่อนจึงจะลบประเภทนี้ได้</small>
                </div>
            `,
        ...
        Swal.fire({
            title: 'ยืนยันการลบ?',
            html: `
                <div class="text-center mb-3">คุณต้องการลบประเภทเครื่องมือนี้ใช่หรือไม่?</div>
                <div class="mt-2 small text-start p-3 bg-light rounded border shadow-sm">
                    <div class="mb-2 pb-2 border-bottom">
                        <span class="text-secondary small">ชื่อประเภทเครื่องมือ:</span><br>
            ...
            preConfirm: () => {
                return $.ajax({
                    url: './api/Master_equipment_category/delete.php',
                    type: 'POST',
                    data: {
                        id: id
                    },
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    ...
                    text: 'ประเภทเครื่องมือถูกลบเรียบร้อยแล้ว',
                    icon: 'success',
                    showConfirmButton: false,
                    timer: 1500,
                    returnFocus: false
    ...
    $(document).on('click', '#btn_export_excel', function(e) {
        ...
        // ยิงไปที่ API
        window.location.href = './api/Master_equipment_category/exportExcel.php?' + formData;
    });
