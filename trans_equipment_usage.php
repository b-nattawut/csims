...
$canManage = hasPermission($pdo, 'equipment_usage.create');
...
try {
    ...
    $current_user_fullname = "เจ้าหน้าที่ผู้ทำรายการ";
}
...
$title = "การใช้งานเครื่องมือทั่วไป";
ob_start();
?>
        ...
        <table class="table table-bordered table-hover table-custom table-striped align-middle mb-0">
            <thead style="font-size: 14px;">
                <tr class="text-nowrap text-center">
                    ...
                    <th class="d-none d-xl-table-cell" style="min-width: 130px;">ประเภทเครื่องมือ</th>
        ...
        <div class="modal-content">
            <div class="modal-header bg-primary text-white py-3 px-4">
                <h5 class="modal-title fw-bold" id="addUsageModalLabel">
                    <i class="fas fa-clipboard-check fa-lg me-2"></i> <span id="textModal">บันทึกประวัติการใช้งานเครื่องมือ</span>
                </h5>
            ...
            <div class="modal-body bg-light p-4">
                    ...
                    <div class="card-body bg-white">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-6 col-lg-6">
                                ...
                                <span class="fw-bold" id="display_usage_category">-</span>
                            </div>
                            <div class="col-12">
                                <span class="d-block text-muted" style="font-size: 0.75rem;">ชื่อเครื่องมือ</span>
                                <span class="fw-bold" id="display_usage_tool_name">-</span>
                            </div>
                            <div class="col-6 col-lg-6">
                ...
                <form method="post" id="equipmentUsageForm" action="./api/Transaction_equipment_usage/save.php" novalidate>
                    ...
                    <fieldset class="bg-white p-4 rounded-3 shadow-sm border">
                        ...
                        <div class="row g-3 justify-content-start">
                            ...
                            <div class="col-12 col-md-4">
                                <label for="usage_date" class="form-label fw-bold">วันที่ใช้งาน <span class="text-danger">*</span></label>
                                <input type="date" id="usage_date" name="usage_date"
                                    class="form-control"
                                    value="<?= date('Y-m-d') ?>" required>
                                <div class="invalid-feedback">กรุณาระบุ วัน เดือน ปี</div>
                            </div>
                            ...
                            <div class="col-12 col-md-4">
                                <label for="condition_status" class="form-label">สภาพเครื่องมือ <span class="text-danger">*</span></label>
                                <select id="condition_status" name="condition_status" class="form-select" required>
                                    <option value="" disabled selected>-- เลือกสภาพเครื่องมือ --</option>
                                    <option value="ปกติ">ปกติ</option>
                                    <option value="ไม่ปกติ">ไม่ปกติ</option>
                                </select>
                                <div class="invalid-feedback">กรุณาเลือกสภาพเครื่องมือ</div>
                            </div>
                            ...
                            <div class="col-12 col-md-8">
                                ...
                                <div class="invalid-feedback">กรุณาระบุ ผู้ใช้งาน</div>
                            </div>
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="input-group input-group-seamless">
                                    <textarea
                                        ...
                                        class="form-control"
                                        rows="2"
                                        placeholder="ระบุหมายเหตุเพิ่มเติม (ถ้ามี) เช่น พบรอยขีดข่วนเล็กน้อย"></textarea>
                                    <button type="button"
                                        class="btn btn-hw-open"
            ...
            <div class="modal-footer d-flex justify-content-end bg-white border-top shadow-sm">
                <button type="submit" class="btn btn-success px-4" id="btn_save_all" form="equipmentUsageForm">
...
<!-- Modal สำหรับดู list รายการใช้งานทั้งหมดของเครื่องมือนั้น ๆ  -->
        ...
        <div class="modal-content">
            <div class="modal-header bg-primary text-white py-3 px-4">
                <h5 class="modal-title fw-bold" id="viewHistoryModalLabel">
                    <i class="fas fa-history me-2"></i> ประวัติการใช้งานเครื่องมือ
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
                    ...
                    <div class="card-body p-3">
                        <div class="row g-3 align-items-end">
                            <div class="col-12 col-md-6">
                                <label for="filter_usage_year" class="form-label text-muted small mb-1 fw-bold"><i class="far fa-calendar me-1"></i> ปี (พ.ศ.)</label>
                                <select id="filter_usage_year" class="form-select form-select-sm">
                            ...
                            <div class="col-12 col-md-6">
                                <label for="filter_usage_month" class="form-label text-muted small mb-1 fw-bold"><i class="far fa-calendar-alt me-1"></i> เดือน</label>
                                <select id="filter_usage_month" class="form-select form-select-sm">
                                    <option value="">-- แสดงทั้งหมด --</option>
                            ...
                            <div class="col-12 mt-4 pt-3 border-top d-flex flex-wrap gap-2 justify-content-center">
                                <button type="button" class="btn btn-warning btn-sm text-dark fw-bold px-4 shadow-sm flex-fill flex-md-grow-0" id="btn_clear_usage_filter">
                                    <i class="fas fa-undo me-2"></i> ล้างค่า
                                </button>
                                <button type="button" class="btn btn-primary btn-sm text-white fw-bold px-4 shadow-sm flex-fill flex-md-grow-0" id="btn_search_usage">
                                    <i class="fas fa-search me-2"></i> ดูข้อมูล
                                </button>
                                <button type="button" class="btn btn-sm fw-bold px-4 shadow-sm flex-fill flex-md-grow-0 text-white" style="background-color: #6f42c1;" id="btn_export_usage_pdf">
...
<script>
    ...
    // ตัวแปรเก็บ ID เครื่องมือปัจจุบัน เพื่อใช้ตอนโหลด/รีเฟรช Modal ประวัติ
    ...
    $(document).ready(function() {
        ...
        $('#filter_usage_year, #filter_usage_month').on('change', function() {
            handleMonthFilter();
        });
    ...
    function handleMonthFilter() {
        const $yearSelect = $('#filter_usage_year');
        const $monthSelect = $('#filter_usage_month');
    ...
    function searchPage(page) {
        ...
        // รอเปลี่ยน API
        $.ajax({
            url: './api/Master_equipment_list/searchDataWithUsage.php',
            type: 'POST',
            data: formData,
            dataType: 'json',
    ...
    function renderTable(data, offset) {
        ...
        if (data && data.length > 0) {
            data.forEach(function(row, i) {
                ...
                let dept = row.department_name || '<span class="text-muted small">ไม่ระบุ</span>';
                ...
                let lastDetail = row.last_report_no ? row.last_report_no : '<span>-</span>';

                // 1. Logic สำหรับปุ่ม Export PDF (สีม่วง)
                ...
                let printBtnHtml = `
                    <button type="button" 
                        class="btn js-print-btn ${btnClass} shadow-sm" 
                        style="${btnStyle}" 
                        data-id="${row.id}" 
                        data-asset="${asset_no}"
                        ${hasLog ? '' : 'disabled'} 
                        title="${hasLog ? 'พิมพ์รายการ F-CS-24' : 'ยังไม่มีประวัติให้พิมพ์'}">
                        <i class="fas fa-file-pdf me-1"></i> Export PDF
                    </button>`;

                html += `
                    <tr class="clickable-row cursor-pointer" data-id="${row.id}" data-category="${category_name}" data-asset="${asset_no}" data-name="${tool_name}" data-department="${row.department_name || '-'}" 
                    data-brand="${row.brand || '-'}" data-model="${row.model || '-'}" data-serial="${row.serial_no || '-'}" data-resp="${responsible}" title="ดูข้อมูล">
                        ...
                        <td class="text-center align-middle">
                            <div class="d-flex align-items-center justify-content-center" style="min-height: 100%;">
                                <button type="button" 
                                        ...
                                        data-id="${row.id}" data-asset="${asset_no}" data-category="${category_name}"
                                        data-department="${row.department_name || '-'}"
                                        data-name="${tool_name}" data-brand="${row.brand || '-'}" data-model="${row.model || '-'}" 
                                        data-serial="${row.serial_no || '-'}" data-resp="${responsible}"
                                        ${window._canManage ? 'title="ลงบันทึกการใช้งานเครื่องมือทั่วไป"' : 'title="คุณไม่มีสิทธิ์จัดการข้อมูล" disabled'}>
                        ...
                        <td class="text-start align-middle d-none d-lg-table-cell">${responsible}</td>
    ...
    // A. กดปุ่ม "ลงบันทึก" จากหน้าตารางหลัก
    $(document).on('click', '.js-add-usage-btn', function() {
        const $form = $('#equipmentUsageForm');

        // Reset ฟอร์มและล้างการตรวจสอบ (Validation)
        $form[0].reset();
        $form.removeClass('was-validated');
        $form.find('.is-invalid, .is-valid').removeClass('is-invalid is-valid');

        // ล้างค่า ID และ Select2 เดิม
        $('#usage_id').val('');
        $('#used_by').val(null).trigger('change');

        $('#equipment_id').val($(this).data('id'));
        $('#display_usage_department').text($(this).data('department'));
        $('#display_usage_category').text($(this).data('category'));
        $('#display_usage_asset_no').text($(this).data('asset'));
        $('#display_usage_tool_name').text($(this).data('name'));
        ...
        // --- Auto Fill ชื่อผู้ใช้ที่ Login อยู่ ---
        const currentUserId = '<?php echo $current_user_id; ?>';
        const currentFullname = '<?php echo $current_user_fullname; ?>';

        if (currentUserId && currentFullname !== "") {
            // เช็คว่ามี Option นี้อยู่แล้วหรือไม่ ถ้าไม่มีให้สร้างใหม่ (ป้องกันกรณี Select2 โหลดไม่ทัน)
        ...
        $('#textModal').text('บันทึกประวัติการใช้งานเครื่องมือ');
        $('#addUsageModalLabel').html('<i class="fas fa-clipboard-check fa-lg me-2"></i> <span id="textModal">บันทึกประวัติการใช้งานเครื่องมือ</span>');
    ...
    // B. กดปุ่ม "บันทึกข้อมูล" ใน Modal Form
    $('#equipmentUsageForm').on('submit', function(e) {
        e.preventDefault();
        const form = this;
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.status == "success") {
                    ...
                    // หากเปิดแฟ้มประวัติอยู่ ให้รีเฟรชข้อมูลในแฟ้มด้วย
                    if ($('#viewHistoryModal').is(':visible') && currentEquipId) {
    ...
    $(document).on('click', '.clickable-row', function(e) {
        ...
        $('#filter_usage_month').val('');
        $('#filter_usage_year').val('');
        handleMonthFilter();

        const $row = $(this);
        currentEquipId = $row.data('id');

        // เซ็ตข้อมูล Header ใน Modal
        $('#hist_category').text($row.data('category'));
        $('#hist_department').text($row.data('department'));
        $('#hist_asset_no').text($row.data('asset'));
        $('#hist_tool_name').text($row.data('name'));
    ...
    // ฟังก์ชันโหลดข้อมูลประวัติมาแสดง
    function loadHistory(equipId) {
        currentEquipId = equipId;
        let month = $('#filter_usage_month').val();
        let year_th = $('#filter_usage_year').val();
        ...
        $.ajax({
            url: './api/Transaction_equipment_usage/getHistory.php',
            type: 'GET',
            data: {
            ...
            success: function(res) {
                if (res.status === 'success') {
                    ...
                    if (res.data && res.data.length > 0) {
                        ...
                        if (month || year_th) {
                            // กรณีที่มีการ Filter แล้วไม่เจอข้อมูล
                            html = `
                                <tr>
                                    <td colspan="6" class="text-center py-5">
                                        <div class="text-muted">
                                            ...
                                            <div class="small opacity-75">
                                                เครื่องมือนี้ยังไม่มีบันทึกการใช้งานใน <b class="text-dark">ปี / เดือน</b> ที่ท่านกำลังค้นหา <br>
                            ...
                            // กรณีที่ไม่มีประวัติเลย (ค่าเริ่มต้น)
                            html = `
                                <tr>
                                    <td colspan="6" class="text-center py-5">
                                        <div class="text-muted">
                                            ...
                                            <small class="opacity-75">เครื่องมือนี้ยังไม่มีข้อมูลการใช้งานในระบบ<br>สามารถเพิ่มรายการใหม่ได้ผ่านปุ่ม <b class="mx-2"><i class="fas fa-circle-info me-1"></i>จัดการข้อมูล</b> ในตารางหลัก</small>
                                        </div>
                                    </td>
                                </tr>`;
                        }
    ...
    $('#btn_clear_usage_filter').on('click', function() {
        $('#filter_usage_month').val('');
        $('#filter_usage_year').val('');
        handleMonthFilter();
        if (currentEquipId) {
    ...
    $('#btn_search_usage').on('click', function() {
        if (currentEquipId) {
            loadHistory(currentEquipId);
        }
    });
    ...
    $('#btn_export_usage_pdf').on('click', function() {
        ...
        // ดึงชื่อรหัสทรัพย์สินจาก Label บนหน้าจอ
        let assetNo = $('#hist_asset_no').text().trim() || 'Equipment';
        let safeName = assetNo.replace(/[/\\?%*:|"<>]/g, '-').replace(/\s+/g, '-');

        // ตรรกะชื่อไฟล์อัจฉริยะ (Smart Naming)
        let namePart = "";
        if (month && year_th) {
        ...
        const fileNamePrefix = "F-CS-24";
        let fileName = `${fileNamePrefix}_${safeName}_${namePart}.pdf`;
        let exportUrl = `./api/Transaction_equipment_usage/gen_pdf.php/${fileName}?equipment_id=${currentEquipId}&month=${month}&year=${year_en}`;

        // เปิดหน้าใหม่เพื่อโหลด PDF
        window.open(exportUrl, '_blank');
    });

    // D. กดแก้ไขประวัติ (จากปุ่มในหน้าตาราง History Modal)
    $(document).on('click', '.js-edit-usage-btn', function() {
        let log = $(this).data('log');

        $('#equipmentUsageForm')[0].reset();
        $('#equipmentUsageForm').removeClass('was-validated');
        ...
        $('#addUsageModalLabel').html('<i class="fas fa-edit fa-lg me-2"></i> <span id="textModal">แก้ไขประวัติการใช้งานเครื่องมือ</span>');
    ...
    $(document).on('click', '.js-delete-usage-btn', function() {
        ...
        Swal.fire({
            ...
            preConfirm: () => {
                return $.ajax({
                    url: './api/Transaction_equipment_usage/delete.php',
                    type: 'POST',
                    data: {
                        id: dataId
                    },
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    ...
                    text: 'ประวัติการใช้งานถูกลบเรียบร้อยแล้ว',
                    icon: 'success',
                    timer: 1500,
                    showConfirmButton: false,
                    returnFocus: false
    ...
    // F. กดปุ่มพิมพ์ประวัติจากหน้าตารางหลัก
    $(document).on('click', '.js-print-btn', function() {
        ...
        let safeName = assetNo.replace(/[/\\?%*:|"<>]/g, '-').replace(/\s+/g, '-');
        const fileNamePrefix = "F-CS-24";
        let fileName = `${fileNamePrefix}_${safeName}_Full-History.pdf`;

        let url = `./api/Transaction_equipment_usage/gen_pdf.php/${fileName}?equipment_id=${equipId}`;
    ...
    $(document).on('click', '#btn_export_excel', function(e) {
        ...
        // ยิงไปที่ API
        window.location.href = './api/Transaction_equipment_usage/exportExcel.php?' + formData;
    });
</script>
<?php
