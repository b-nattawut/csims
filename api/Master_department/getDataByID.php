...
// -------------------------------------------------------------------------
// 3. Query ข้อมูลจากตาราง master_departments
// -------------------------------------------------------------------------
$sql = "SELECT 
            t1.id,
            t1.department_name,
            
            -- นับจำนวนการใช้งานแยกตามแต่ละตาราง (Usage Count Breakdown)
            (SELECT COUNT(id) FROM master_equipment_list WHERE department_id = t1.id AND status_delete = 0) AS usage_equipment,
            (SELECT COUNT(id) FROM master_computer_list WHERE department_id = t1.id AND status_delete = 0) AS usage_computer,
            (SELECT COUNT(id) FROM master_temperature_device WHERE department_id = t1.id AND status_delete = 0) AS usage_thermometer,
            (SELECT COUNT(id) FROM master_chemical_list WHERE department_id = t1.id AND status_delete = 0) AS usage_chemical,
...
try {
    ...
    if ($data) {
        // คำนวณยอดรวมทั้งหมด (Total Usage) จากผลลัพธ์ย่อยทั้ง 4
        $data['total_usage'] = $data['usage_equipment'] + $data['usage_computer'] + $data['usage_thermometer'] + $data['usage_chemical'];
        ...
        echo json_encode([
            'status' => 'error',
            'message' => 'ไม่พบข้อมูลหน่วยงานนี้ หรือรายการอาจถูกลบไปแล้ว'
        ], JSON_UNESCAPED_UNICODE);
    }
