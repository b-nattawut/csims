...
$department_name = $_GET['filter_department_name'] ?? '';
...
// 4. SQL Query 
// ดึงข้อมูลหลัก + Subquery นับจำนวนรวม + ข้อมูล Audit
$sql = "SELECT 
            t1.id, 
            t1.department_name,
            
            -- นับจำนวนยอดรวมที่ใช้งาน 
...
try {
    ...
    $sheet->setTitle('รายชื่อหน่วยงาน');
    ...
    foreach ($data as $index => $row) {
        ...
        $sheet->setCellValue('B' . $rowNumber, $row['department_name']);
    ...
    $filename = "Departments_" . date('Y-m-d_H.i') . ".xlsx";
