...
try {
    ...
    // 4. Query ข้อมูล (ดึงทั้งหมดที่ตรง Filter - ตัด LIMIT ออก)
    $sql = "SELECT 
                t1.id, 
    ...
    $sheet->setTitle('การซ่อมบำรุงสอบเทียบเครื่องมือ');
    ...
    $headers = [
        ...
        'J1' => 'วันที่ซ่อมบำรุงล่าสุด',
        'K1' => 'การซ่อมบำรุงล่าสุด',
        'L1' => 'ผู้สร้างรายการ',
        'M1' => 'วันที่สร้าง',
        'N1' => 'ผู้แก้ไขล่าสุด',
    ...
    foreach ($data as $index => $row) {
        ...
        $sheet->setCellValue('J' . $rowNumber, $row['last_log_date'] ?: 'ยังไม่มีประวัติ');
        $sheet->setCellValue('K' . $rowNumber, $row['last_log_detail'] ?: 'ยังไม่มีประวัติ');
        ...
        // จัดรูปแบบกึ่งกลางและเส้นขอบ
        $currentRange = 'A' . $rowNumber . ':O' . $rowNumber;
    ...
    // 8. ดาวน์โหลดไฟล์
    $filename = "Equipment_Maintenance_Log_" . date('Y-m-d_H.i') . ".xlsx";
