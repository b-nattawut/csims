...
try {
    ...
    // 3. ตั้งค่าเงื่อนไข WHERE (อิงตาม searchData)
    ...
    $sql = "SELECT 
        ...
        -- รวมล็อตและชื่อกลุ่มย่อยสารตั้งต้นกันแถวข้อมูลในสเปรดชีตระเบิดซ้ำบรรทัด
    ...
    -- เจาะข้ามผ่านตารางกลางเชื่อมประวัติ
    ...
    -- JOIN เข้าหาตารางหน่วยนับของสารที่เตรียมมาตรวจ
    ...
    GROUP BY 
    t1.id, 
        t1.amount_used, t1.ph_value, t1.color_change_status, t1.readiness_status, t1.is_verified, t1.remark, t1.test_date, t1.create_date, t1.update_date,
        t2.prep_name,
        t5.unit_name_th, t5.unit_symbol, t5.unit_name_en, t5.unit_group,
    ...
    $sheet->setTitle('รายการตรวจความพร้อมสารเคมี');

    // กำหนดหัวตาราง (A ถึง R)
    $headers = [
        'A1' => 'ลำดับ',
        'B1' => 'วันที่ทดสอบ',
    ...
    foreach ($data as $index => $row) {
        ...
        $colorStatus = ($row['color_change_status'] == 'changed') ? "เปลี่ยนสี" : "ไม่เปลี่ยนสี";
        $readiness = ($row['readiness_status'] == 'ready') ? "พร้อมใช้งาน" : "ไม่พร้อมใช้งาน";
        $verifyStatus = ($row['is_verified'] == 1) ? "ทวนสอบแล้ว" : "รอการทวนสอบ";
        ...
        $sheet->setCellValue('H' . $rowNumber, $row['ph_value'] ?: '-');
        $sheet->setCellValue('I' . $rowNumber, $colorStatus ?: '-');
        $sheet->setCellValue('J' . $rowNumber, $readiness ?: '-');
        $sheet->setCellValue('K' . $rowNumber, $row['tester_fullname'] ?: '-');
        $sheet->setCellValue('L' . $rowNumber, $row['verifier_fullname'] ?: '-');
        $sheet->setCellValue('M' . $rowNumber, $verifyStatus ?: '-');
        $sheet->setCellValue('N' . $rowNumber, $row['remark'] ?: '-');
        $sheet->setCellValue('O' . $rowNumber, $row['creator_name'] ?: '-');
        $sheet->setCellValue('P' . $rowNumber, $row['created_at'] ?: '-');
        $sheet->setCellValue('Q' . $rowNumber, $row['updater_name'] ?: '-');
        $sheet->setCellValue('R' . $rowNumber, $row['updated_at'] ?: '-');

        // จัดรูปแบบกึ่งกลางและเส้นขอบ (A ถึง R)
    ...
    // 8. ส่งไฟล์ออก
    $filename = "Latent_Chemical_Test_Report_" . date('Y-m-d_H.i') . ".xlsx";
