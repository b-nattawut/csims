...
try {
    ...
    $test_start         = $_POST['filter_test_start'] ?? '';
    $test_end           = $_POST['filter_test_end'] ?? '';
    ...
    $filter_ph          = $_POST['filter_ph'] ?? '';
    $filter_color       = $_POST['filter_color'] ?? '';
    $readiness_status   = $_POST['filter_readiness'] ?? ''; // ready / not_ready
    ...
    if ($user_role !== 'admin') {
        ...
        if (!empty($filter_dept_id) && $filter_dept_id != $user_dept_id) {
            http_response_code(403);
            echo json_encode(['status' => 'error', 'message' => 'ไม่มีสิทธิ์เข้าถึงข้อมูลการตรวจความพร้อมสารเคมีของหน่วยงานอื่น']);
            exit;
        }
    ...
    // 6. Query ข้อมูลหลัก (JOIN เพื่อให้ได้รายละเอียดครบถ้วนสำหรับตาราง)
    $sql = "SELECT 
            t1.id,
            t1.is_verified,
            t1.amount_used,
            ...
            -- มัดกลุ่มสารตั้งต้นป้องกันแถวแตกเบิ้ล
        ...
        -- เชื่อมต่อผ่านตารางย่อยสูตรผสมใหม่
    ...
    // 7. วนลูปปั้นข้อมูลหน่วยนับ (Logic มาตรฐานระบบ CSIMS)
    $finalData = [];
