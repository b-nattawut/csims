<?php
/**
 * API: Save/Update Fingerprint Report Data
 * บันทึกข้อมูลร่างรายงานตรวจเก็บวัตถุพยาน (ลายนิ้วมือแฝง)
 * ลงคอลัมน์ incident_report_data ใน incident_checklist_transaction (key: fingerprint)
 */
...
try {
    ...
    if (!$existing) {
        $pdo->rollBack();
        echo json_encode(['success' => false, 'message' => 'ไม่พบข้อมูล กรุณาบันทึก checklist ก่อน'], JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    // เก็บข้อมูลใน key 'fingerprint'
    $currentReportData['fingerprint'] = $formData;
    ...
    echo json_encode([
        'success' => true,
        'message' => 'บันทึกร่างรายงานตรวจเก็บวัตถุพยาน (ลายนิ้วมือแฝง) เรียบร้อยแล้ว',
        'count_report' => $countEdit
