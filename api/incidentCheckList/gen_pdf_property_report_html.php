...
$displayOverride = getVal($prop, 'rp_report_no_display_pdf');
...
// Agency short - always use DB-derived values to prevent incorrect overlap
$agencyShort = !empty($agencyName) ? $agencyName : getVal($prop, 'rp_agency_name', '');
$agencyTypeShort = !empty($agencyType) ? $agencyType : getVal($prop, 'rp_agency_type', '');
...
// ==========================================
// 6. EXPORT สำหรับ DOCX (คดีทรัพย์) — ไม่ต้อง render PDF template
// ==========================================
if (defined('WORD_DOCX_MODE') && WORD_DOCX_MODE) {
    $GLOBALS['property_report_export'] = [
