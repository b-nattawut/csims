...
try {
    ...
    $sql = "SELECT 
            ...
            -- ข้อมูลจากการทดสอบ (Format วันที่)
            ...
            -- ใช้ GROUP_CONCAT รวบรวมข้อมูลสารตั้งต้นทั้งหมดที่มีการผสมกัน
        ...
        -- วิ่งทะลุตารางกลางเข้าคลังสินค้าเพื่อหาประวัติสารผสม
    ...
    if ($data) {
        ...
        // --- 4. Logic การจัดการหน่วยนับให้ "กริบ" เหมือนหน้าอื่นๆ ---
        $unitDisplay = '-';
        ...
        // เพิ่มฟิลด์ unit_display เข้าไปในก้อนข้อมูล
        $data['unit_display'] = $unitDisplay;

        // ปั้นค่า text ให้เป็นรูปแบบเดียวกับที่เราชอบ
        $nbsp = "\u{00A0}";
        ...
        echo json_encode([
            'status' => 'error',
            'message' => 'ไม่พบข้อมูลการทดสอบสารเคมีนี้ หรืออาจถูกลบไปแล้ว'
        ]);
    }
